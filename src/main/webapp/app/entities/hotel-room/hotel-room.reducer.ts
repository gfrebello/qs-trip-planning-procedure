...
export type HotelRoomState = Readonly<typeof initialState>;

// Reducer

export default (state: HotelRoomState = initialState, action): HotelRoomState => {
  switch (action.type) {
...
export const getEntities: ICrudGetAllAction<IHotelRoom> = (page, size, sort) => ({
  type: ACTION_TYPES.FETCH_HOTELROOM_LIST,
  payload: axios.get<IHotelRoom>(`${apiUrl}?cacheBuster=${new Date().getTime()}`)
});

export const getEntity: ICrudGetAction<IHotelRoom> = id => {
  const requestUrl = `${apiUrl}/${id}`;
  return {
    type: ACTION_TYPES.FETCH_HOTELROOM,
    payload: axios.get<IHotelRoom>(requestUrl)
  };
};

export const createEntity: ICrudPutAction<IHotelRoom> = entity => async dispatch => {
...
export const updateEntity: ICrudPutAction<IHotelRoom> = entity => async dispatch => {
  const result = await dispatch({
...
export const deleteEntity: ICrudDeleteAction<IHotelRoom> = id => async dispatch => {
  const requestUrl = `${apiUrl}/${id}`;
  const result = await dispatch({
